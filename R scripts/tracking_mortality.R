...
download.file(paste0('https://www.ons.gov.uk/file?uri=%2fpeoplepopulationandcommunity%2fhealthandsocialcare%2fcausesofdeath%2fdatasets%2fdeathregistrationsandoccurrencesbylocalauthorityandhealthboard%2f2020/lahbtablesweek3721092020170523.xlsx'),  paste0(github_repo_dir, '/Source files/ons_mortality.xlsx'), mode = 'wb')
...
download.file('https://www.ons.gov.uk/file?uri=%2fpeoplepopulationandcommunity%2fbirthsdeathsandmarriages%2fdeaths%2fdatasets%2fdeathsinvolvingcovid19bylocalareaanddeprivation%2f1march2020to30june2020/referencetablesworkbook2.xlsx', paste0(github_repo_dir, '/granular_mortality_file.xlsx'), mode = 'wb')

la_asmr <- read_excel(paste0(github_repo_dir, '/granular_mortality_file.xlsx'), sheet = "Table 2", skip = 5, col_names = c('Cause', 'Sex',' Area_type', 'Code', 'Name', 'null_1', 'March_deaths', 'March_asmr_val', 'null_2', 'March_asmr_lci', 'March_asmer_uci', 'null_3', 'April_deaths', 'April_asmr_val', 'null_4', 'April_asmr_lci', 'April_asmer_uci', 'null_5', 'May_deaths', 'May_asmr_val', 'null_6', 'May_asmr_lci','May_asmr_uci', 'null_7', 'June_deaths', 'June_asmr_val', 'null_8', 'June_asmr_lci', 'June_asmr_uci', 'null_9','Four_month_deaths', 'Four_month_asmr_val', 'null_10', 'Four_month_asmr_lci', 'Four_month_asmr_uci'),  col_types = c('text', 'text','text', 'text', 'text', 'text', 'numeric', 'numeric', 'text', 'numeric', 'numeric', 'text', 'numeric', 'numeric', 'text', 'numeric', 'numeric', 'text', 'numeric', 'numeric', 'text', 'numeric','numeric', 'text', 'numeric', 'numeric', 'text', 'numeric', 'numeric','text', 'numeric', 'numeric', 'text', 'numeric', 'numeric')) %>% 
 select(-c(null_1,null_2,null_3,null_4,null_5, null_6,null_7, null_8, null_9, null_10)) %>%
  filter(!is.na(Name)) %>% 
  gather(key = 'item', value = 'value', March_deaths:Four_month_asmr_uci) %>% 
  mutate(Month = ifelse(grepl('March', item), 'March',ifelse(grepl('April', item), 'April',ifelse(grepl('May', item), 'May',ifelse(grepl('June', item), 'June',ifelse(grepl('Four_month', item), 'March - June 2020', NA)))))) %>% 
...
msoa_deaths <- read_excel(paste0(github_repo_dir, '/granular_mortality_file.xlsx'), sheet = "Table 5", skip = 13, col_names = c('Code', 'Name', 'HOC_name','null_1','March_Covid','April_Covid','May_Covid','June_Covid','Four_months_Covid','null_2','March_Non_Covid','April_Non_Covid','May_Non_Covid','June_Non_Covid','Four_months_Non_Covid','null_3','March_deaths','April_deaths','May_deaths','June_deaths','Four_month_deaths'), col_types = c("text","text", "text", 'text', "numeric", "numeric",'numeric','numeric','numeric','text','numeric','numeric','numeric','numeric','numeric','text','numeric','numeric','numeric','numeric','numeric')) %>% 
  select(-c(null_1,null_2,null_3)) %>% 
  gather(key = 'item', value = 'value', March_Covid:Four_month_deaths) %>% 
  mutate(Month = ifelse(grepl('March', item), 'March',ifelse(grepl('April', item), 'April',ifelse(grepl('May', item), 'May',ifelse(grepl('June', item), 'June',ifelse(grepl('Four_month', item), 'March - June 2020', NA)))))) %>% 
